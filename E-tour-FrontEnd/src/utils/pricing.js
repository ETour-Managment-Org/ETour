...
export function isDomestic(tour) {
  return String(tour && tour.tourType).toUpperCase() === 'DOMESTIC'
}

export function oddTravellerChoiceNeeded(count) {
  return count >= 3 && count % 2 === 1
}

export function roomTypeLabel(rooming) {
  if (!rooming) return '-'

  const { twinSharing = 0, single = 0, extraBeds = 0 } = rooming
  const parts = []

  if (twinSharing > 0) {
    parts.push(twinSharing === 2 ? 'Twin sharing' : `Twin sharing x ${twinSharing}`)
  }
  if (single > 0) {
    parts.push(single === 1 ? 'Single room' : `${single} single rooms`)
  }
  if (extraBeds > 0) {
    parts.push(extraBeds === 1 ? '1 extra bed' : `${extraBeds} extra beds`)
  }
