...
export default function TourDetailPage() {
  ...
  useEffect(() => {
    ...
    tourApi
      ...
      .then((t) => {
        setTour(t)
        const open = (t.schedules || []).find((s) => (s.availableSeats ?? 0) > 0)
  ...
  const selected = (tour.schedules || []).find((s) => s.scheduleId === Number(scheduleId))
  const bookable = (tour.schedules || []).filter((s) => (s.availableSeats ?? 0) > 0)
    ...
    <div className="container page">
      <Breadcrumb
        items={[
          ...
          ...trail.map((c) => ({ label: c.categoryName, to: `/categories/${c.categoryId}` })),
          { label: tour.tourName }
        ]}
      />
      ...
      <div className="grid grid-side">
          ...
          <div className="card mb24">
            <div className="thumb">
              <img
                ...
                alt={`${tour.tourName} in ${tour.destination || ''}`}
                onError={onImageError}
              />
            </div>
            <div className="card-pad">
              {tour.categoryName && <span className="badge badge-accent mb8">{tour.categoryName}</span>}
              <h1 className="mb8">{tour.tourName}</h1>
              <div className="center wrap" style={{ gap: 14 }}>
                <span className="small muted">{tour.destination}</span>
              ...
              {gallery.length > 1 && (
                <div className="gallery">
                  {gallery.slice(0, 4).map((img) => (
                    <img key={img.imageId} src={img.url} alt={img.imageTitle || tour.tourName}
            ...
            <div className="card card-pad">
              ...
              <table className="table mb24">
                ...
                <tbody>
                  {(tour.costs || []).map((c) => (
                    <tr key={c.costId}>
                      <td className="strong">{inr(c.adultPrice)}</td>
                      <td>{inr(c.singlePersonPrice)}</td>
                      <td>{inr(c.extraPersonPrice)}</td>
                      <td>{inr(c.childWithBedPrice)}</td>
                      <td>{inr(c.childWithoutBedPrice)}</td>
                      <td className="small muted">{prettyDate(c.validFrom)} - {prettyDate(c.validTo)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <>
                  ...
                  {(reviews.reviews || []).map((r) => (
                    <div key={r.reviewId} className="mb16" style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
                      ...
                      <div className="tiny muted mt8">{prettyDate(r.reviewDate)}</div>
                    </div>
                  ))}
                </>
              )}
        ...
        <aside className="card card-pad sticky-side">
          ...
          <div className="price" style={{ fontSize: 32, lineHeight: 1.2 }}>
            {inr(activeCost?.adultPrice || tour.price)}
          </div>
            ...
            <select className="select" value={scheduleId} onChange={(e) => setScheduleId(e.target.value)}>
              ...
              {bookable.map((s) => (
                <option key={s.scheduleId} value={s.scheduleId}>
                  {prettyDate(s.startDate)} — {s.availableSeats} seats
                </option>
              ))}
            </select>
            {bookable.length === 0 && <div className="err-text">All departures are sold out.</div>}
          </div>
